...
fn run_attested_trial(
    ...
    // Step 2: Extra crypto operations (measured separately)
    let commitments_start = Instant::now();
    // commitments already computed in run_attested_setup_crypto_only
    let commitments_ms = commitments_start.elapsed().as_millis() as u64;
    
    let pk_from_commitments_start = Instant::now();
    // pk from commitments already computed in run_attested_setup_crypto_only  
    let pk_from_commitments_ms = pk_from_commitments_start.elapsed().as_millis() as u64;
    
    let transcript_digest_start = Instant::now();
    // transcript digest already computed in run_attested_setup_crypto_only
    let transcript_digest_ms = transcript_digest_start.elapsed().as_millis() as u64;

...
pub fn run_attested_setup_crypto_only<E: Pairing, R: rand::RngCore>(
    ...
    // Time commitment computation
    let commitments_start = Instant::now();
    let raw_commitments = compute_commitments::<E>(&shares);
    let commitments_ms = commitments_start.elapsed().as_millis() as u64;
    
    // Time PK from commitments
    let pk_start = Instant::now();
    let pk = compute_pk_from_commitments::<E>(&inp.share_domain, &raw_commitments.share_commitments)?;
    let pk_from_commitments_ms = pk_start.elapsed().as_millis() as u64;
    
    let commitments = DealerCommitments { pk, share_commitments: raw_commitments.share_commitments };
    ...
    // Time transcript digest computation
    let digest_start = Instant::now();
    let transcript_digest = digest_transcript::<E>(&crs, &commitments, &meta);
    let transcript_digest_ms = digest_start.elapsed().as_millis() as u64;
